...
private:
    ...
    {
        return _stricmp(item.name.c_str(), "Connection") == 0;
    }
        ...
        while( begin != end )
        {
            ...
            case HeaderValue:
                if( input == '\r' )
                {
                    ...
                    else if(_stricmp(h.name.c_str(), "Transfer-Encoding") == 0 )
                    {
                        if(_stricmp(h.value.c_str(), "chunked") == 0)
                            chunked = true;
                    }
                    state = ExpectingNewline_2;
                }
            ...
            case ExpectingNewline_3: {
                ...
                {
                    if(_stricmp(it->value.c_str(), "Keep-Alive") == 0 )
                    {
                        resp.keepAlive = true;
                    }
                    else  // == Close
